...
### Track C 加強（P2 — Web exploitation）

- **Burp Suite（或課程指定代理）**：至少完成 **1 次**手動請求修改（參數／Header）並保存 **raw request** 節錄（可打馬賽克 cookie）。
- （選）在講師核准下，對照 **XSS／SSRF** 其中一類做**紙上或靶機**演練敘述（不需真打外網）。
